...
function BySkuForm({ bomQty, matsForSku, lotsFor, autoCost, unitFor, invName, factorFor, potes, tolling, onAdded, initial, editBatch, onCancelEdit }: {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-sm p-5 space-y-4">
      ...
      <div>
        <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold mb-1">MOC (opcional · repartí las cases en uno o varios)</p>
        <div className="space-y-1">
          {mocs.map((m, i) => (
...
function ProductionTab({ fpMovements, ipMovements, onAdded }: {
  ...
  const ipStock = useMemo(() => {
    ...
    for (const r of ipMovements) {
      ...
      const cur = map.get(key) ?? { lot, qty: 0, cost: null as number | null, unit: r.unit ?? "lbs", material: r.material, firstDate: null as string | null } as any;
      ...
      if (r.type === "In" && r.movement_date && (!cur.firstDate || r.movement_date < cur.firstDate)) cur.firstDate = r.movement_date;
...
function ProcurementTab({ movements, orders, baseline, ipMovements, onAdded }: { movements: FPRow[]; orders: any[]; baseline: BaselineRow[]; ipMovements: IPRow[]; onAdded: () => void }) {
  ...
  async function publishIngPrices() {
    const { error } = await supabase.from("ops_published" as any).upsert({ key: "ingredient_prices", value: ingPrices, published_at: new Date().toISOString() });
  ...
  function setBomQtyCell(sku:string, mat:string, val:number){
    ...
    // Save to Supabase
    supabase.from("ops_bom" as any).upsert(
  ...
  function setBomPctCell(sku:string, mat:string, pct:number){
    setBomQty(prev=>{
      ...
      // Save to Supabase
      supabase.from("ops_bom" as any).upsert({ sku, material: mat, qty_per_case: newVal }, { onConflict: "sku,material" })
        .then(({ error }) => { if (error) console.error("BOM save error:", error); });
  ...
  function saveRawMatField(material: string, field: string, value: any) {
    supabase.from("ops_raw_materials" as any).upsert(
  ...
  async function renameMaterial() {
    ...
    await supabase.from("ops_raw_materials" as any).delete().eq("material", oldName);
    const { error: e1 } = await supabase.from("ops_raw_materials" as any).insert({
    ...
    if (bomRows && bomRows.length > 0) {
      ...
      await supabase.from("ops_bom" as any).delete().eq("material", oldName);
    }
    // Update forecast PO references
    await supabase.from("ops_forecast_po" as any).update({ material: newName }).eq("material", oldName);
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await supabase.from("ops_forecast_po" as any).select("*").order("id");
  ...
  async function confirmIpForecastPO() {
    ...
    await supabase.from("ops_forecast_po" as any).delete().eq("id", po.id);
  ...
  async function updateIpForecastPO(id: number, field: string, value: any) {
    ...
    supabase.from("ops_forecast_po" as any).update({
  ...
  function updateWip(sku:string, patch:Partial<{cases:string;due:string}>){
    setWip(w=>{
      ...
      supabase.from("ops_wip" as any).upsert({
