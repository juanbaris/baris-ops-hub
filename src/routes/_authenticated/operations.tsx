...
function FPStockTab({ movements, orders, loading }: { movements: FPRow[]; orders: any[]; loading: boolean }) {
  ...
  const forecastNextMonth = useMemo(() => {
    ...
    return Object.fromEntries(SKUS.map((sku) => [sku, bySkuMonthKey[sku]?.[key] ?? FORECAST_FALLBACK[sku]])) as Record<
      SKU,
      number
    >;
  }, [bySkuMonthKey]);
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {SKUS.map((sku) => {
          ...
          return (
            <div
              key={sku}
              className={`rounded-2xl border p-4 text-center shadow-sm ${isCrit ? "border-red-200 bg-red-50" : isLow ? "border-orange-200 bg-orange-50" : "border-border bg-card"}`}
            >
              ...
              <p
                className={`text-xl font-bold font-mono mt-1 ${isCrit ? "text-red-600" : isLow ? "text-orange-500" : ""}`}
                style={!isCrit && !isLow ? { color: "#1C2340" } : {}}
              >
                {available.toLocaleString()}
              </p>
              <p className="text-[10px] text-muted-foreground">available cases</p>
              <p className="text-[11px] font-mono font-semibold mt-0.5" style={{ color: "#1C2340" }}>
                {woh.toFixed(1)} wks
              </p>
              <span className={`inline-block mt-1 rounded-full px-2 py-0.5 text-[9px] font-bold ${STATUS_PILL[st]}`}>
                {st}
              </span>
            </div>
          );
                ...
                .map((s) => {
                  ...
                  return (
                    <tr key={`${s.sku}|${s.warehouse}`} className="border-t border-border/60 hover:bg-muted/20">
                      <td className="px-4 py-2 font-semibold" style={{ color: "#1C2340" }}>
                        {s.sku}
                      </td>
...
function FPInputTab({ movements, loading, onAdded }: { movements: FPRow[]; loading: boolean; onAdded: () => void }) {
  const [form, setForm] = useState({
    ...
    sku: "XD" as SKU,
    cases: "",
    warehouse: "Lineage Newark" as Warehouse,
    lot_number: "",
    concept: "Production" as FPConcept,
    cogs_per_case: "",
    po_number_ref: "",
    notes: "",
  });
  ...
  async function save() {
    if (!form.cases || Number(form.cases) <= 0) {
      toast.error("Cases required");
      return;
    }
    if (!form.lot_number && form.concept === "Production") {
      toast.error("Lot number required for Production");
      return;
    }
    setSaving(true);
  ...
  const filtered = useMemo(() => {
    return [...movements]
      ...
      .sort((a, b) =>
        sortDir === "desc" ? (a.movement_date < b.movement_date ? 1 : -1) : a.movement_date > b.movement_date ? 1 : -1,
      );
  }, [movements, filterSku, filterType, sortDir]);

  const inp =
    "rounded-lg border border-border bg-background px-3 py-1.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-primary/30";
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
          <div>
            <label className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Date</label>
            <input
              type="date"
              className={`${inp} mt-1`}
          ...
          <div>
            <label className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">SKU</label>
            <select className={`${inp} mt-1`} value={form.sku} onChange={(e) => set("sku", e.target.value)}>
              {SKUS.map((s) => (
                <option key={s} value={s}>
                  {s} ({SKU_ITEMS[s as SKU]})
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Cases *</label>
            <input
              type="number"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
          <div>
            <label className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Warehouse</label>
          ...
          <div>
            ...
            <input
              ...
              placeholder={form.concept === "Production" ? "Required" : "Optional"}
            />
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Concept</label>
          ...
          <div>
            ...
            <input
              type="number"
              className={`${inp} mt-1 font-mono`}
              value={form.cogs_per_case}
              onChange={(e) => set("cogs_per_case", e.target.value)}
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            <label className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">PO Ref</label>
            <input
              className={`${inp} mt-1 font-mono`}
              value={form.po_number_ref}
              onChange={(e) => set("po_number_ref", e.target.value)}
              placeholder="Optional"
            />
          </div>
          <div>
        ...
        >
          {saving
            ...
            : editingFP
              ? "Update movement"
              : `+ Add ${form.type} · ${form.cases || "?"} cases ${form.sku}`}
        </button>
        {editingFP && (
          <button
            onClick={cancelEditFP}
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-border bg-muted/30 flex items-center justify-between gap-3 flex-wrap">
          <p className="text-sm font-semibold" style={{ color: "#1C2340" }}>
            FP Movements <span className="text-muted-foreground font-normal text-xs">({filtered.length} records)</span>
          </p>
              ...
              filtered.map((r) => (
                <tr key={r.id} className="border-t border-border/60 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-1.5">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${r.type === "In" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                    >
                      {r.type}
                    </span>
                  </td>
                  <td className="px-4 py-1.5 font-semibold" style={{ color: "#1C2340" }}>
                    {r.sku}
                  </td>
                  ...
                  <td className="px-4 py-1.5 font-mono text-xs" style={{ color: "#A3224A" }}>
                    {r.lot_number}
                  </td>
...
function IPInputTab({ movements, loading, onAdded }: { movements: IPRow[]; loading: boolean; onAdded: () => void }) {
  const [form, setForm] = useState({
    ...
    unit: "lbs",
    lot_number: "",
    concept: "Procurement" as IPConcept,
    warehouse: "Heinlein",
    total_price: "",
    shipping_price: "",
    other_costs: "",
  ...
  async function save() {
    ...
    toast.success(
      editing ? "I&P movement updated" : `Added: ${form.type} ${form.quantity} ${form.unit} ${form.material}`,
    );
    setEditing(null);
    setForm((f) => ({
      ...f,
  ...
  const SortTh = ({ col, label }: { col: "date" | "material" | "qty"; label: string }) => (
    <th
      className="px-3 py-2.5 text-left cursor-pointer select-none hover:text-foreground"
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
          ...
          <div>
            ...
            <select className={`${inp} mt-1`} value={form.unit} onChange={(e) => set("unit", e.target.value)}>
              {["lbs", "kg", "Piece", "cases", "units"].map((u) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <div>
            <Lbl>Est. receive date</Lbl>
            <input
              type="date"
              className={`${inp} mt-1`}
              value={form.estimated_receive_date}
              onChange={(e) => set("estimated_receive_date", e.target.value)}
            />
          </div>
          <div>
            <Lbl>Est. payment date</Lbl>
            <input
              type="date"
              className={`${inp} mt-1`}
              value={form.estimated_payment_date}
              onChange={(e) => set("estimated_payment_date", e.target.value)}
            />
          </div>
              ...
              filtered.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-t border-border/60 hover:bg-muted/20">
                    ...
                    <td className="px-3 py-1.5 font-semibold" style={{ color: "#1C2340" }}>
                      {r.material}
                    </td>
                    ...
                    <td className="px-3 py-1.5" style={{ color: "#A3224A" }}>
                      {r.vendor ?? "—"}
                    </td>
                      ...
                      {confirmId === r.id ? (
                        <span className="flex flex-col items-end gap-1">
                          <span className="flex items-center gap-1.5 text-xs">
                            ...
                            <button
                              onClick={() => remove(r.id)}
                              className="rounded bg-red-600 px-2 py-0.5 text-[10px] font-semibold text-white"
                            >
                              Confirm
                            </button>
                            <button
...
function IPSummaryTab({ movements }: { movements: IPRow[] }) {
  ...
  const shown = filterMaterial === "all" ? inventory : inventory.filter((m) => m.material === filterMaterial);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-sm">
        <div className="px-5 py-3 border-b border-border bg-muted/30">
          <p className="text-sm font-bold" style={{ color: "#1C2340" }}>
            I&P Inventory — current stock by material & lot
          </p>
          <p className="text-xs text-muted-foreground">
            Net balance from all movements · COGS = weighted average of In movements
          </p>
        </div>
        <table className="w-full text-xs min-w-max">
          <thead>
                ...
                return (
                  <React.Fragment key={m.material}>
                    <tr className="border-t-2 border-border bg-muted/10 font-semibold">
                      <td className="px-4 py-2" style={{ color: "#1C2340" }}>
                        {m.material}
                      </td>
                      ...
                      <td className="px-4 py-2 text-right font-mono">{avgCogs > 0 ? `$${avgCogs.toFixed(4)}` : "—"}</td>
                    ...
                    {lots.map(([lot, v]) => (
                      <tr key={`${m.material}|${lot}`} className="border-t border-border/40 hover:bg-muted/20">
                        ...
                        <td className="px-4 py-1.5 font-mono" style={{ color: "#A3224A" }}>
                          {lot}
                        </td>
          ...
          <tfoot>
            <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
              <td className="px-4 py-2 font-semibold text-xs" colSpan={5}>
...
const SCRAP: Record<string, number> = { rasp: 0.1, choc: 0.08, other: 0 };
...
const BOM: Record<string, BomLine[]> = {
  XD: [
    ...
    { material: "Cocoa Butter", unit: "lbs", pct: 0.0, scrapGroup: "other" },
    { material: "Soy Lecithin", unit: "lbs", pct: 0.0, scrapGroup: "other" },
  ...
  HM: [
    ...
    { material: "Hazelnut Butter", unit: "lbs", pct: 0.1, scrapGroup: "other" },
  ...
  WM: [
    { material: "IQF Rasp", unit: "lbs", pct: 0.3, scrapGroup: "rasp" },
    { material: "Corinthian White", unit: "lbs", pct: 0.388, scrapGroup: "choc" },
    { material: "Valcour Milk", unit: "lbs", pct: 0.3, scrapGroup: "choc" },
  ...
  WD: [
    { material: "IQF Rasp", unit: "lbs", pct: 0.3, scrapGroup: "rasp" },
    { material: "RASG Dark 72%", unit: "lbs", pct: 0.3, scrapGroup: "choc" },
    { material: "Corinthian White", unit: "lbs", pct: 0.388, scrapGroup: "choc" },
    { material: "Cocoa Butter", unit: "lbs", pct: 0.012, scrapGroup: "other" },
    { material: "Soy Lecithin", unit: "lbs", pct: 0.001, scrapGroup: "other" },
...
function FPTransferForm({ fpMovements, onAdded }: { fpMovements: FPRow[]; onAdded: () => void }) {
  ...
  async function save() {
    if (!form.sku || !form.cases || !form.lot) {
      toast.error("SKU, lot and cases required");
      return;
    }
    if (!form.from_wh || !form.to_wh) {
      toast.error("From and To warehouses required");
      return;
    }
    if (form.from_wh === form.to_wh) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-sm p-5">
      <h3 className="text-sm font-bold mb-1" style={{ color: "#1C2340" }}>
        FP Transfer between warehouses
      </h3>
      <p className="text-xs text-muted-foreground mb-4">
        Creates one OUT + one IN movement · COGS carries over (+ freight if entered)
      </p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
        <div>
          <Lbl>Date</Lbl>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
        <div>
          <Lbl>From warehouse</Lbl>
          <select
            className={`${inp} mt-1`}
            value={form.from_wh}
            onChange={(e) => setForm((f) => ({ ...f, from_wh: e.target.value }))}
          >
            {FP_WAREHOUSES_ALL.map((w) => (
              <option key={w} value={w}>
                {w}
              </option>
            ))}
        ...
        <div>
          <Lbl>Freight/case ($) · optional</Lbl>
          <input
            type="number"
            step="0.01"
            className={`${inp} mt-1 font-mono`}
            value={form.freight_per_case}
            placeholder="0.00"
            onChange={(e) => setForm((f) => ({ ...f, freight_per_case: e.target.value }))}
          />
        </div>
        <div>
          <Lbl>Notes</Lbl>
      ...
      {form.lot && (
        <div className="mb-4 rounded-xl bg-muted/30 border border-border px-4 py-2.5 flex gap-6 text-xs">
          <span>
            Source COGS:{" "}
            <span className="font-mono font-bold" style={{ color: "#1C2340" }}>
              {lotCogs != null ? `$${lotCogs.toFixed(2)}/case` : "—"}
            </span>
          </span>
          {freight > 0 && (
            <span>
              + Freight: <span className="font-mono font-bold text-orange-600">${freight.toFixed(2)}/case</span>
            </span>
          )}
          {newCogs != null && (
            <span className="ml-auto font-semibold">
              COGS at destination: <span className="font-mono text-emerald-700">${newCogs.toFixed(2)}/case</span>
            </span>
          )}
        </div>
      )}
  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      <div className="px-5 py-3 border-b border-border bg-muted/30 flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-sm font-bold" style={{ color: "#1C2340" }}>
            Inventory history — closing stock by month
          </p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs min-w-max">
          ...
          <tbody>
            {shownMaterials.map((mat) => (
              <tr key={mat} className="border-t border-border/40 hover:bg-muted/20">
                <td className="px-4 py-1.5 font-semibold sticky left-0 bg-card" style={{ color: "#1C2340" }}>
                  {mat}
                </td>
                {history.map((snap) => {
                  ...
                  return (
                    <td
                      key={snap.month}
                      className={`px-3 py-1.5 text-right font-mono ${val < 0 ? "text-red-600 font-semibold" : ""}`}
                    >
                      {viewMode === "units"
          ...
          {filterMaterial === "all" && (
            <tfoot>
              <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
                <td className="px-4 py-2 font-semibold text-xs sticky left-0" style={{ backgroundColor: "#1C2340" }}>
                  TOTAL VALUE
                </td>
                {history.map((snap) => {
          ...
          messages: [
            {
              ...
              content: [
                ...
                {
                  type: "text",
                  text: `Extract production run data from this Heinlein production report. Return ONLY JSON (no markdown):
{
  "run_date": "YYYY-MM-DD",
  ...
  const bomLines = useMemo(() => {
    ...
    return bom
      .filter((l) => (l.pct ?? 0) > 0 || (l.perCase ?? 0) > 0)
      .map((line) => {
  ...
  async function save() {
    if (!form.cases_produced || cases <= 0) {
      toast.error("Cases produced required");
      return;
    }
    if (!form.lot_number) {
    ...
    if (finalCogs <= 0) {
      toast.error("COGS could not be calculated — enter override COGS manually");
      return;
    }
    setSaving(true);
    ...
    if (editingRun) {
      ...
      const { data: runData, error: runErr } = await supabase
        ...
        .single();
      if (runErr || !runData) {
        toast.error(runErr?.message ?? "Failed");
        setSaving(false);
        return;
      }
      ...
      for (const line of bomLines) {
        ...
        await supabase.from("ip_movements").insert({
          ...
          quantity: Math.round(line.qty * 100) / 100,
          unit: line.unit,
  ...
  function startEdit(r: any) {
    ...
    setForm({
      ...
      sku: r.sku as SKU,
      cases_produced: String(r.cases_produced),
      lot_number: r.lot_number ?? "",
      notes: r.notes ?? "",
      override_cogs: String(r.cogs_per_case),
    });
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
      ...
      {activeForm === "production" && (
        <div className="rounded-2xl border border-border bg-card shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            ...
            <div>
              <Lbl>Cases Produced *</Lbl>
              <input
                type="number"
                className={`${inp} mt-1 font-mono`}
                value={form.cases_produced}
                min={1}
                onChange={(e) => set("cases_produced", e.target.value)}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
            ...
            <div>
              ...
              <input
                ...
                step="0.01"
                className={`${inp} mt-1 font-mono`}
                value={form.override_cogs}
                placeholder={autoCogs > 0 ? `Auto: $${autoCogs.toFixed(2)}` : "—"}
                onChange={(e) => set("override_cogs", e.target.value)}
              />
            </div>
            <div>
              <Lbl>Notes</Lbl>
              <input className={`${inp} mt-1`} value={form.notes} onChange={(e) => set("notes", e.target.value)} />
            </div>
          </div>
            ...
            <div className="rounded-xl border border-border bg-muted/20 p-3 mb-4">
                  ...
                  <tbody>
                    {bomLines.map((line) => {
                      ...
                      return (
                        <tr
                          key={line.material}
                          className={`border-t border-border/40 ${!enough ? "bg-red-50/30" : ""}`}
                        >
                          <td className="py-1 font-semibold" style={{ color: "#1C2340" }}>
                            {line.material}
                          </td>
              ...
              {form.override_cogs && (
                <p className="mt-2 text-[10px] text-orange-600 font-semibold">
                  ⚠ Using manual COGS override of ${Number(form.override_cogs).toFixed(2)}/case instead of calculated $
                  {autoCogs.toFixed(2)}/case
                </p>
              )}
            </div>
          )}
          ...
          <div className="flex gap-2">
            ...
            {!editingRun && (
              <p className="text-xs text-muted-foreground self-center">
                ↳ Creates FP IN + {bomLines.length} IP OUT movements
              </p>
            )}
          </div>
        </div>
              ...
              runs.map((r) => (
                <tr key={r.id} className="border-t border-border/60 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-1.5 text-right font-mono font-semibold">
                    {Number(r.cases_produced).toLocaleString()}
                  </td>
                  <td className="px-4 py-1.5 text-right font-mono text-muted-foreground">
                    ${Number(r.cogs_per_case).toFixed(2)}
                  </td>
                    ...
                    {confirmId === r.id ? (
                      <span className="flex flex-col items-end gap-1">
                        <span className="text-[10px] text-amber-600 font-semibold">
                          ⚠ Also deletes linked FP + IP movements
                        </span>
                        <span className="flex items-center gap-1.5 text-xs">
                          Delete?
                          <button
                      ...
                      <span className="flex justify-end gap-2">
                        ...
                        <button onClick={() => setConfirmId(r.id)} className="text-muted-foreground hover:text-red-600">
                          🗑
                        </button>
                      </span>
                    )}
...
const BOM_DATA: Record<string, { lbs_per_case: number; ingredients: Record<string, number> }> = {
  ...
  HM: {
    lbs_per_case: 2.5,
    ingredients: { "IQF Raspberry": 0.825, Chocolate: 1.398, "Hazelnut Butter": 0.25, "Cocoa Butter": 0.024 },
  },
  WD: { lbs_per_case: 2.5, ingredients: { "IQF Raspberry": 0.75, Chocolate: 1.719, "Cocoa Butter": 0.029 } },
  WM: { lbs_per_case: 2.5, ingredients: { "IQF Raspberry": 0.75, Chocolate: 1.719, "Cocoa Butter": 0.029 } },
  Matcha: {
    lbs_per_case: 2.5,
    ingredients: { "IQF Raspberry": 1.125, Chocolate: 1.322, "Matcha Powder": 0.022, "Cocoa Butter": 0.025 },
  },
};
          ...
          <div className="divide-y divide-border/60">
            ...
            <div className="flex items-center justify-between px-5 py-2.5 bg-muted/10">
              ...
              <input
                ...
                step="0.01"
                value={tolling}
                onChange={(e) => setTolling(parseFloat(e.target.value) || 0)}
        ...
        <div className="space-y-4">
                ...
                <div key={b.ingredient} className="flex items-center justify-between px-5 py-2 hover:bg-muted/20">
                  <div>
                    ...
                    <span className="ml-2 text-xs text-muted-foreground font-mono">
                      {b.lbs.toFixed(3)} lbs × ${b.price.toFixed(2)}
                    </span>
                  </div>
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm p-4">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">
              Gross Margin preview at standard price
            </p>
            {[
              ["UNFI/KeHe", 36.96],
              ["RFD", 38.5],
              ...
              return (
                ...
                >
                  <span className="text-sm">
                    {dist as string} · ${price as number}/case
                  </span>
                  <span
                    className={`font-mono text-sm font-bold ${gm > 30 ? "text-emerald-600" : gm > 15 ? "text-orange-500" : "text-red-500"}`}
                  >
                    {gm.toFixed(1)}% GM
                  </span>
                </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {SKUS.map((sku) => {
              ...
              const defaultRM = Object.entries(BOM_DATA[sku]?.ingredients ?? {}).reduce(
                (s, [ing, lbs]) => s + lbs * (DEFAULT_PRICES[ing] ?? 3),
                0,
              );
              ...
              return (
                <tr key={sku} className="border-t border-border/60 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-2 text-right font-mono font-bold" style={{ color: "#1C2340" }}>
                    ${r.total.toFixed(2)}
                  </td>
                  <td
                    className={`px-4 py-2 text-right font-mono text-xs ${Math.abs(delta) < 0.01 ? "text-muted-foreground" : delta > 0 ? "text-red-500" : "text-emerald-600"}`}
                  >
                    {Math.abs(delta) < 0.01 ? "—" : (delta > 0 ? "+" : "") + delta.toFixed(2)}
                  </td>
                  <td
                    className={`px-4 py-2 text-right font-mono font-semibold ${gm > 30 ? "text-emerald-600" : gm > 15 ? "text-orange-500" : "text-red-500"}`}
                  >
                    {gm.toFixed(1)}%
                  </td>
                </tr>
...
const DEFAULT_ING_PRICES: Record<string, number> = {
  "IQF Raspberry": 2.91,
  "RASG Dark 72%": 5.5,
  "Corinthian White": 3.2,
  "Valcour Milk": 5.2,
  "Duluth Dark": 4.88,
  "Pistachio Paste": 17.23,
  "Hazelnut Paste": 12.2,
  "Matcha Powder": 19.5,
  Spirulina: 11.88,
  "Cocoa Butter": 6.5,
...
const DEFAULT_PROD_COSTS = {
  tolling_per_unit: 0.65,
  cup_per_unit: 0.095,
  lid_per_unit: 0.092,
  sealer_per_unit: 0.03,
  case_per_case: 0.36,
};
const ING_PACK_SIZES: Record<string, number> = {
  "IQF Raspberry": 22,
  "RASG Dark 72%": 1100,
  "Corinthian White": 1100,
  "Valcour Milk": 1100,
  "Duluth Dark": 1100,
  "Pistachio Paste": 550,
  "Hazelnut Paste": 550,
...
const SKU_MIX_PCT: Record<string, number> = { XD: 0.3, PW: 0.25, HM: 0.18, WM: 0.12, WD: 0.08, Matcha: 0.07 };
...
function buildOpsForecast(bySkuMonthKey: Record<string, Record<string, number>>): Record<string, number[]> {
  return Object.fromEntries(SKUS.map((sku) => [sku, FORECAST_KEYS_OPS.map((k) => bySkuMonthKey[sku]?.[k] ?? 0)]));
}
...
function CommittedRequirements() {
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = "production-requirements.csv";
    a.click();
  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-border bg-muted/30">
        <div>
          <p className="text-sm font-bold" style={{ color: "#1C2340" }}>
            Production requirements — {isCommitted ? "committed scenario" : "active forecast"}
          </p>
          {isCommitted ? (
            ...
            <p className="text-xs text-muted-foreground">
              ℹ️ No committed scenario — showing active forecast ({scenario}). Go to Sales → Simulador and click SET on
              any lever to define the production input.
            </p>
          )}
        </div>
        <div className="flex items-center gap-2">
              ...
              <tr key={m.label} className="border-t border-border/60 hover:bg-muted/20">
                ...
                {SKUS.map((s) => (
                  <td key={s} className="px-3 py-1.5 text-right font-mono">
                    {(m.skuBreakdown[s] ?? 0).toLocaleString()}
                  </td>
                ))}
                {newSkuNames.map((n) => (
                  <td key={n} className="px-3 py-1.5 text-right font-mono">
                    {(m.newSkuBreakdown.find((x) => x.name === n)?.cases ?? 0).toLocaleString()}
                  </td>
                ))}
...
function calcCOGSFull(
  prices: Record<string, number>,
  costs: typeof DEFAULT_PROD_COSTS,
  scrap: { raspberry: number; chocolate: number },
) {
  return Object.fromEntries(
    SKUS.map((sku) => {
      ...
      const pkg =
        costs.cup_per_unit + costs.lid_per_unit + costs.sealer_per_unit + costs.case_per_case / UNITS_PER_CASE_BOM;
      const per_unit =
        rasp / UNITS_PER_CASE_BOM +
        choc / UNITS_PER_CASE_BOM +
        other / UNITS_PER_CASE_BOM +
        pkg +
        costs.tolling_per_unit;
      return [
        sku,
        {
          rasp: rasp / UNITS_PER_CASE_BOM,
          choc: choc / UNITS_PER_CASE_BOM,
          other: other / UNITS_PER_CASE_BOM,
          pkg,
          tolling: costs.tolling_per_unit,
          per_unit,
          per_case: per_unit * UNITS_PER_CASE_BOM,
        },
      ];
    }),
  );
  ...
  for (const sku of SKUS) {
    ...
    for (let i = 0; i < FORECAST_MONTHS_OPS.length; i++) {
      ...
      if (isWindow) {
        ...
        for (let j = i; j < Math.min(i + freqMonths * 2, FORECAST_MONTHS_OPS.length); j++)
          lookahead -= FORECAST_SKU_OPS[sku]?.[j] ?? 0;
        if (lookahead < 0 || woh < safetyWoh) {
          const demand = FORECAST_MONTHS_OPS.slice(i, i + freqMonths).reduce(
            (s, _, j) => s + (FORECAST_SKU_OPS[sku]?.[i + j] ?? 0),
            0,
          );
      ...
      if (produce > 0) {
        ...
        for (const [ing, pct] of Object.entries(bom)) {
          ...
          const qty = lbs * (isR ? 1.1 : isC ? 1.08 : 1);
...
function ProcurementTab({ movements, orders }: { movements: FPRow[]; orders: any[] }) {
  ...
  const [scrap, setScrap] = useState({ raspberry: 0.1, chocolate: 0.08 });
  ...
  function updateWip(sku: string, patch: Partial<{ cases: string; due: string }>) {
    setWip((w) => {
      ...
      try {
        window.localStorage.setItem(WIP_KEY, JSON.stringify(next));
      } catch {
        /* ignore */
      }
      return next;
  ...
  const { plan, stockProj, ingNeeded, ingByMonth } = useMemo(
    () => calcProdSchedule(bySku, orders, safetyWoh, minRun, freqMonths, fcstOps, wipBySku),
    [bySku, orders, safetyWoh, minRun, freqMonths, fcstOps, wipBySku],
  );
  ...
  const ingWindow = useMemo(() => {
    ...
    for (const [ing, arr] of Object.entries(ingByMonth)) {
      let s = 0;
      for (let i = shopRange[0]; i <= shopRange[1]; i++) s += arr[i] ?? 0;
      if (s > 0) out[ing] = s;
    }
    return out;
  }, [ingByMonth, shopRange]);
  const shopCasesWindow = shopRange ? totalByMonth.slice(shopRange[0], shopRange[1] + 1).reduce((a, b) => a + b, 0) : 0;
  ...
  const inp =
    "rounded border border-border bg-background px-2 py-1 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-primary/30";
  const SUBTABS: { id: ProcSubTab; label: string }[] = [
    { id: "schedule", label: "📅 Schedule" },
    { id: "stock_proj", label: "📊 Stock Projection" },
        ...
        <div className="flex flex-wrap items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              step={500}
              value={minRun}
              onChange={(e) => setMinRun(Number(e.target.value))}
          ...
          <div className="ml-auto flex gap-6 text-center">
            <div>
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Total to produce</p>
              <p className="text-xl font-bold font-mono" style={{ color: "#A3224A" }}>
                {totalProduce.toLocaleString()} cases
              </p>
            </div>
            <div>
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide">COGS ponderado</p>
              <p className="text-xl font-bold font-mono" style={{ color: "#1C2340" }}>
                ${weightedCOGS.toFixed(2)}/case
              </p>
            </div>
          </div>
        </div>
      ...
      {procTab === "schedule" && (
        <div className="space-y-3">
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs text-amber-700">
            🏭 Yellow cells = planned runs · Frequency:{" "}
            {["", "monthly", "bimonthly", "quarterly", "four-monthly", "", "semiannual"][freqMonths]} · Safety{" "}
            {safetyWoh}w · Min {minRun.toLocaleString()} cases
          </div>
          <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-xs text-blue-700">
            🛠️ "In production now" = cases currently being manufactured. They count as available stock, so the planner
            shifts or shrinks the suggested runs. Once the run is finished, log it in Production (creates the real In
            movement) and clear the cell here.
          </div>
          <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-sm">
            <table className="text-xs min-w-max w-full">
              <thead>
                <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
                  <th className="px-4 py-2.5 text-left sticky left-0" style={{ backgroundColor: "#1C2340" }}>
                    SKU
                  </th>
                  ...
                  {FORECAST_MONTHS_OPS.map((m) => (
                    <th key={m} className="px-3 py-2.5 text-center min-w-[75px]">
                      {m}
                    </th>
                  ))}
              ...
              <tbody>
                {SKUS.map((sku) => {
                  ...
                  return (
                    <tr key={sku} className="border-t border-border/60 hover:bg-muted/20">
                      <td className="px-4 py-1.5 font-semibold sticky left-0 bg-card" style={{ color: "#1C2340" }}>
                        {sku}{" "}
                        <span className="text-muted-foreground font-normal text-[10px]">({SKU_ITEMS[sku as SKU]})</span>
                      </td>
                      ...
                      <td
                        ...
                        style={{ color: wipCases > 0 ? "#10B981" : undefined }}
                      >
                      ...
                      {(plan[sku] ?? []).map((prod, i) => (
                        <td
                          key={i}
                          className={`px-3 py-1.5 text-center font-mono font-semibold ${prod > 0 ? "text-amber-900" : "text-muted-foreground"}`}
                      ...
                      <td
                        ...
                        style={{ color: skuTotal > 0 ? "#A3224A" : "#10B981" }}
                      >
                ...
                <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
                  <td className="px-4 py-2 font-semibold sticky left-0 text-xs" style={{ backgroundColor: "#1C2340" }}>
                    Total cases
                  </td>
                  ...
                  {totalByMonth.map((t, i) => (
                    <td
                      key={i}
                      className="px-3 py-2 text-center font-mono font-bold text-amber-300"
      ...
      {procTab === "stock_proj" && (
        <div className="space-y-3">
                    ...
                    {(stockProj[sku] ?? []).map((stock, i) => {
                      ...
                      return (
                        <td
                          ...
                          style={{
                            backgroundColor: isCrit ? "#FEE2E2" : isLow ? "#FEF3C7" : isProd ? "#DCFCE7" : undefined,
                            color: isCrit ? "#DC2626" : isLow ? "#92400E" : "#1C2340",
          ...
          <div className="flex gap-3 text-xs flex-wrap">
            {[
              ["bg-red-100", "🔴 Critical (< 2w or negative)"],
              ["bg-yellow-100", `🟡 Low (< ${safetyWoh}w safety)`],
              ["bg-green-100", "🟢 Production month"],
            ].map(([cls, label]) => (
              <div key={label} className={`flex items-center gap-1.5 rounded px-3 py-1 ${cls}`}>
                <span>{label}</span>
              </div>
            ))}
          </div>
      ...
      {procTab === "bom_cogs" && (
        <div className="space-y-5">
          <div className="rounded-2xl border border-border bg-card p-4 shadow-sm flex flex-wrap gap-6 items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                min={0}
                max={0.5}
                value={scrap.raspberry}
                onChange={(e) => setScrap((s) => ({ ...s, raspberry: parseFloat(e.target.value) || 0 }))}
                className={`${inp} w-16`}
              />
            </div>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                min={0}
                max={0.5}
                value={scrap.chocolate}
                onChange={(e) => setScrap((s) => ({ ...s, chocolate: parseFloat(e.target.value) || 0 }))}
                className={`${inp} w-16`}
              />
            </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-sm">
            <div className="px-5 py-3 border-b border-border bg-muted/30">
              <p className="text-sm font-bold" style={{ color: "#1C2340" }}>
                Formula (BOM) — % Receta · Source: Super BOM Consolidado
              </p>
                  ...
                  <tr key={ing} className="border-t border-border/60 hover:bg-muted/20">
                    ...
                    {SKUS.map((sku) => {
                      ...
                      return (
                        <td
                          key={sku}
                          className={`px-3 py-1.5 text-center font-mono ${pct > 0 ? "font-semibold" : "text-muted-foreground"}`}
                        >
                          {pct > 0 ? `${pct.toFixed(1)}%` : "—"}
                        </td>
                      );
          ...
          <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            <p className="text-xs font-semibold text-muted-foreground mb-3 uppercase tracking-wide">
              Tolling + Packaging ($/unit)
            </p>
            <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
              {[
                ["Tolling (Heinlein)", "tolling_per_unit"],
                ["Cup", "cup_per_unit"],
                ["Lid", "lid_per_unit"],
                ["Sealer", "sealer_per_unit"],
                ["Case $/case", "case_per_case"],
              ].map(([label, key]) => (
                <div key={key}>
                  ...
                  <input
                    type="number"
                    step="0.001"
                    value={prodCosts[key as keyof typeof prodCosts]}
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            ...
            <table className="w-full text-sm">
                ...
                {SKUS.map((sku) => {
                  ...
                  return (
                    <tr key={sku} className="border-t border-border/60 hover:bg-muted/20">
                      ...
                      <td className="px-4 py-2 text-right font-mono font-bold" style={{ color: "#1C2340" }}>
                        ${c.per_unit.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-right font-mono font-bold" style={{ color: "#A3224A" }}>
                        ${c.per_case.toFixed(2)}
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
                  <td className="px-4 py-2 font-semibold text-xs" colSpan={6}>
                    COGS PONDERADO (portfolio mix)
                  </td>
                  <td className="px-4 py-2 text-right font-mono font-bold text-emerald-400">
                    ${(weightedCOGS / UNITS_PER_CASE_BOM).toFixed(2)}
                  </td>
                  <td className="px-4 py-2 text-right font-mono font-bold text-emerald-400">
                    ${weightedCOGS.toFixed(2)}
                  </td>
                </tr>
              </tfoot>
            </table>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card px-4 py-3 shadow-sm">
            ...
            {(
              [
                ["next", "Next run only"],
                ["3m", "Next 3 months"],
                ["all", "Full horizon (12 mo)"],
              ] as const
            ).map(([id, label]) => (
              <button
                key={id}
                onClick={() => setShopScope(id)}
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-xs text-blue-700">
            {shopScope === "next" && nextRunIdx >= 0
              ...
              : ""}
            💡 Inventory = load actual stock in the "Raw Materials" tab. To Acquire = Needed − Inventory rounded to pack
            size.
          </div>
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <table className="w-full text-sm">
                  ...
                  return (
                    <tr key={ing} className="border-t border-border/60 hover:bg-muted/20">
                      ...
                      <td className="px-4 py-2 text-right font-mono text-muted-foreground">
                        {inv > 0 ? inv.toLocaleString() : "—"}
                      </td>
                      <td
                        className={`px-4 py-2 text-right font-mono ${toAcq > 0 ? "font-semibold text-orange-600" : ""}`}
                      >
                        {toAcq > 0 ? toAcq.toLocaleString() : "✓"}
                      </td>
                      <td className="px-4 py-2 text-right font-mono text-muted-foreground">{ps.toLocaleString()}</td>
                      <td className="px-4 py-2 text-right font-mono font-bold" style={{ color: "#1C2340" }}>
                        {finalAmt > 0 ? finalAmt.toLocaleString() : "—"}
                      </td>
                      ...
                      <td
                        ...
                        style={{ color: cost > 0 ? "#A3224A" : "#10B981" }}
                      >
              ...
              <tfoot>
                <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
                  <td className="px-4 py-2 font-semibold text-xs" colSpan={7}>
                    TOTAL INGREDIENTS
                  </td>
                  <td className="px-4 py-2 text-right font-mono font-bold text-emerald-400">
                    $
                    {ALL_INGS.filter((ing) => (ingWindow[ing] ?? 0) > 0)
                      .reduce((s, ing) => {
                        const needed = Math.round(ingWindow[ing] ?? 0);
                        const inv = parseInt(ingInv[ing]) || 0;
                        const toAcq = Math.max(0, needed - inv);
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <table className="w-full text-sm">
                ...
                {ALL_INGS.map((ing) => {
                  ...
                  return (
                    <tr key={ing} className="border-t border-border/60 hover:bg-muted/20">
                      <td className="px-4 py-2 font-medium">{ing}</td>
                      <td className="px-4 py-2 text-right font-mono text-muted-foreground">
                        {(ING_PACK_SIZES[ing] ?? 0).toLocaleString()}
                      </td>
                      ...
                      <td className="px-4 py-2 text-right">
                        <input
                          ...
                          placeholder="0"
                          className={`${inp} w-28 text-right ${inv > 0 ? "bg-emerald-50" : ""}`}
                        />
                      </td>
                      <td className="px-4 py-2 text-right font-mono text-muted-foreground">
                        {inv > 0 ? `$${(inv * price).toLocaleString()}` : "—"}
                      </td>
              ...
              <tfoot>
                <tr style={{ backgroundColor: "#1C2340", color: "#fff" }}>
                  <td className="px-4 py-2 font-semibold text-xs" colSpan={3}>
                    TOTAL INVENTORY
                  </td>
                  <td className="px-4 py-2 text-right font-mono">
                    {ALL_INGS.reduce((s, ing) => s + (parseInt(ingInv[ing]) || 0), 0).toLocaleString()} lbs
                  </td>
                  <td className="px-4 py-2 text-right font-mono font-bold text-emerald-400">
                    $
                    {ALL_INGS.reduce((s, ing) => {
                      const inv = parseInt(ingInv[ing]) || 0;
                      return s + inv * (ingPrices[ing] ?? 0);
                    }, 0).toLocaleString()}
                  </td>
                  <td />
                </tr>
...
function OperationsPage() {
  ...
  return (
    <>
      ...
      {tab === "production" && (
        <ProductionTab fpMovements={fpMovementsAll} ipMovements={ipMovements} onAdded={loadAll} />
      )}
