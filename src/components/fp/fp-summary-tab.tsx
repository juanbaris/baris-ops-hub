...
export function FPSummaryTab() {
  ...
  function exportCsv() {
    ...
    const data: (string | number)[][] = [
      ...
      [
        "TOTAL",
        Math.round(tot.openingValue),
        Math.round(tot.inValue),
        Math.round(tot.outValue),
        Math.round(tot.closingValue),
        "",
      ],
    ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card">
        <div
          className="border-b border-border bg-muted/30 px-5 py-2.5 text-sm font-semibold"
          style={{ color: "#1C2340" }}
        >
          Cases
        </div>
        <table className="w-full min-w-max text-sm">
          <thead className="bg-muted/20">
            <tr>
              ...
              {["Opening", "+ IN", "− OUT", "= Closing", "Change"].map((h) => (
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.sku} className="border-t border-border/60 hover:bg-muted/20">
                <td className="px-3 py-2 font-semibold" style={{ color: "#1C2340" }}>
                  {SKU_LABEL[r.sku]}
                </td>
          ...
          <tfoot>
            <tr className="border-t-2 border-border bg-muted/30 font-semibold">
              <td className="px-3 py-2" style={{ color: "#1C2340" }}>
                TOTAL
              </td>
              ...
              <td className="px-3 py-2 text-right font-mono">
                {(tot.closingCases - tot.openingCases).toLocaleString()}
              </td>
            </tr>
          </tfoot>
        </table>
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card">
        <div
          ...
          style={{ color: "#1C2340" }}
        >
          Dollar value (COGS-based)
        </div>
        <table className="w-full min-w-max text-sm">
          <thead className="bg-muted/20">
            <tr>
              <th className={`${th} text-left`}>SKU</th>
              {["Opening $", "+ IN $", "− OUT $", "= Closing $"].map((h) => (
                <th key={h} className={`${th} text-right`}>
                  {h}
                </th>
              ))}
              ...
              <tr key={sku} className="border-t border-border/40 hover:bg-muted/20">
                ...
                {history.map((snap) => {
                  ...
                  return (
                    ...
                    >
                      {viewMode === "cases" ? val.toLocaleString() : val ? `$${Math.round(val).toLocaleString()}` : "—"}
                    </td>
                  );
                })}
              </tr>
